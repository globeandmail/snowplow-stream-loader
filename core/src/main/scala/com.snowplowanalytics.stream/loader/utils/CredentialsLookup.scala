...
object CredentialsLookup {
  ...
  def getCredentialsProvider(
    ...
    (arnRole, stsRegion) match {
      case (Some(arnRoleValue), Some(stsRegionValue)) =>
        val stsClient = AWSSecurityTokenServiceClientBuilder
          .standard()
          .withCredentials(provider)
          .withRegion(stsRegionValue)
          .build()

        val assumedRoleSTSProvider =
          new STSAssumeRoleSessionCredentialsProvider.Builder(arnRoleValue, "sophi-stream-loader")
            .withStsClient(stsClient)
            .build()

        assumedRoleSTSProvider
      case _ => provider
    }
  }
